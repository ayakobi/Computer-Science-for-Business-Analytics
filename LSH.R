...
for(i in 1:10){
  sample = unique(sample(ncol(structureddata),ncol(structureddata), replace = TRUE))
  train = structureddata[,sample]
  test = structureddata[,-sample]
  
  for(j in 1:length(bands)){
    Fmeasure[j] = LSH(train, 480, bands[j]) 
  }
...
Fmeasures = matrix(NA,5,length(bands))
...
for(i in 1:10){
  ...
  for(j in 1:length(bands)){
    Fmeasures[i,j] = LSH(train, 480, bands[j]) 
  }
  
}

#Plot of the average F results for all the bootstraps
t = bands^(-(bands/480))
...
for(i in 1:10){
  ...
  for(j in 1:length(thresholds)){
    Fmeasure[j] = Clustering(train, 480, 80, thresholds[j]) 
  }
